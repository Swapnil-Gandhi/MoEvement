...
class compression_scheduler():
    ...
    def state_dict(self):
        """Snapshot the dynamic compression-schedule state.

        The per-technique ``TECHNIQUE_ENABLED`` toggles are recomputed from
        ``training_steps`` on the next ``.step()`` call, so preserving just
        the step counter and the ``weight_quantization_enabled`` latch is
        enough for a MoEvement peer-pull spare to land on the same
        schedule state as its surviving DP peers.
        """
        return {
            "training_steps": self.training_steps,
            "weight_quantization_enabled": self.weight_quantization_enabled,
        }

    def load_state_dict(self, state):
        """Inverse of ``state_dict``; tolerates a partial dict."""
        if state is None:
            return
        self.training_steps = state.get("training_steps", self.training_steps)
        self.weight_quantization_enabled = state.get("weight_quantization_enabled", self.weight_quantization_enabled)
