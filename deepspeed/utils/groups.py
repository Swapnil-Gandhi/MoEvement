...
def _create_expert_and_data_parallel(expert_parallel_size_, use_data_before_expert_parallel_=False):
    """
        ...
        Topology-aware: when ``mpu`` exposes ``dp_groups`` (the
        PipelineParallelGrid), the expert / expert-data groups are
        partitioned WITHIN each DP group from the topology rather than
        derived from rank-index arithmetic.  Without this the index-based
        derivation assumes a pipe-major rank layout (ranks within a PP
        stage are contiguous), which silently picks the wrong axis under
        data-major topology — expert-DP collapses onto PP partners
        instead of DP partners, mathematically corrupting MoE expert
        gradient averaging in normal training and deadlocking under
        MoEvement recovery when PP partners are paused.
    """
    assert dist.is_initialized()
    ...
    # Topology-aware DP groups: prefer ``mpu.dp_groups`` (a list of
    # all per-pipe-stage DP groups from the PipelineParallelGrid) over
    # rank-index arithmetic.  Without the topology these blocks fall
    # back to the original layout assumption.
    topo_dp_groups = None
    if mpu is not None and hasattr(mpu, "dp_groups"):
        # Each entry is a list/tuple of world ranks forming one DP group.
        topo_dp_groups = [list(g) for g in mpu.dp_groups]

    # Only create group if it does not already exist
    if group_name not in _EXPERT_DATA_PARALLEL_GROUP:
        if topo_dp_groups is not None:
            # Within each topology DP group, partition into expert_parallel_size_
            # contiguous chunks.  Each chunk is one expert_parallel set; the
            # corresponding-position ranks across chunks form the expert-DP set
            # (mirrors the use_data_before_expert_parallel_ layout but on the
            # actual DP rank list, not consecutive world ranks).
            for dp_ranks in topo_dp_groups:
                _ensure_divisibility(len(dp_ranks), expert_parallel_size_)
                ep_dp_size = len(dp_ranks) // expert_parallel_size_
                for i in range(expert_parallel_size_):
                    if use_data_before_expert_parallel_:
                        ranks = dp_ranks[i * ep_dp_size:(i + 1) * ep_dp_size]
                    else:
                        ranks = dp_ranks[i::expert_parallel_size_]
                    group = dist.new_group(ranks)
                    log_dist(
    ...
    if group_name not in _EXPERT_PARALLEL_GROUP:
        if topo_dp_groups is not None:
            for dp_ranks in topo_dp_groups:
                ...
                ep_dp_size = len(dp_ranks) // expert_parallel_size_
                if use_data_before_expert_parallel_:
                    # Cross-chunk grouping: index i within each chunk forms
                    # an expert-parallel group of ``expert_parallel_size_`` ranks.
                    for i in range(ep_dp_size):
                        ranks = dp_ranks[i::ep_dp_size]
                        group = dist.new_group(ranks)
                        log_dist(
                    ...
                    # Contiguous chunks of expert_parallel_size_ within each DP group.
                    for i in range(ep_dp_size):
                        ranks = dp_ranks[i * expert_parallel_size_:(i + 1) * expert_parallel_size_]
                        group = dist.new_group(ranks)
                        log_dist(
                            f'creating expert parallel process group named {group_name} '
                            f'with ranks: {list(ranks)}', [0])
                        if rank in ranks:
                            _EXPERT_PARALLEL_GROUP[group_name] = group
                            _EXPERT_PARALLEL_GROUP_RANKS[group_name] = ranks
        elif use_data_before_expert_parallel_:
            for pp_stage_start in range(0, world_size, pp_stride):
...
def _clone_world_group():
    """Return the default WORLD process group, cached in ``_WORLD_GROUP``.

    Historically this called ``dist.new_group(ranks=range(world_size))``
    to get a PG distinct from ``dist.group.WORLD`` — justified at the
    time by ``dist.get_global_rank()`` not accepting the WORLD sentinel.
    On torch 2.6+ ``get_global_rank(dist.group.WORLD, rank)`` returns
    cleanly (verified empirically), so the clone is unnecessary and its
    ``new_group`` call causes a store-keyed bootstrap whose index drifts
    between ranks under ``survivor_rendezvous`` (survivors call it once
    from rebuild's ``_refresh_group_caches``; the cold-started spare
    calls it from inside ``deepspeed.initialize``).  The two call
    positions produce NCCL comms that can't communicate with each other,
    breaking every post-rebuild collective.

    Using ``dist.group.WORLD`` (a sentinel that always resolves to the
    current default PG) removes the counter-drift failure mode.  After
    a ``destroy_process_group`` + re-init the sentinel resolves to the
    NEW default PG, so this still works correctly across rebuilds.

    Behavior change: DeepSpeed's internal collectives now share the
    default PG's NCCL comm with any user-issued collective on
    ``dist.group.WORLD`` — previously they were isolated.  No
    in-repo user-side concurrent WORLD collectives exist (audited
    2026-04-20), so the change has no regression surface here.
    """
    assert dist.is_initialized(), "dist is not initialized"
    global _WORLD_GROUP
...
def _get_local_all_to_all_group():
    ...
    # Idempotency guard: the other init helpers in this module short-circuit
    # on "already populated" (e.g. ``_EXPERT_PARALLEL_GROUP`` checks membership
    # before constructing, ``_clone_world_group`` checks ``is None``) so a
    # second call after first-time init is a no-op.  This function used to
    # lack that guard and would re-issue ``dist.new_group`` on identical rank
    # sets, silently leaking the originals (and tripping NCCL duplicate-group
    # detection after a rebuild).  Mirror the other helpers so
    # ``reset_for_rebuild`` + re-run works cleanly.
...
def reset_for_rebuild(destroy_fn=None):
    """Abort + clear every cached sub-group so re-init can run against a fresh world.

    Normally DeepSpeed's sub-groups are built once at engine init and the
    caches above short-circuit subsequent init attempts with "already
    initialised" checks (e.g. ``_ZERO_PARAM_INTRA_PARALLEL_GROUP`` asserts
    ``None`` before construction, ``_EXPERT_PARALLEL_GROUP`` skips if the
    key exists).  That's the right default for the single-init lifecycle,
    but it blocks re-init after a spare-rank substitution: the fresh
    world has different rank membership, but the cached globals still
    reference the old (now-dead) communicators.

    This helper is the rebuild-path hook: it tears down every cached
    ProcessGroup reference via ``destroy_fn`` and nulls the module
    globals so the next call to ``_init_tp_mesh_device`` /
    ``_create_expert_and_data_parallel`` / ``_clone_world_group`` /
    ``_get_local_all_to_all_group`` / ``_create_zero_param_parallel_group``
    rebuilds cleanly.

    Only the *group-holding* globals are touched.  ``mpu`` (externally-
    provided Megatron object), ``expert_tensor_parallel_world_size``
    (int, not a group), and ``mesh_device`` (torch device) are left
    alone — they are not stale-after-topology-change in the same way.

    Args:
        destroy_fn: optional callable ``(pg) -> None`` used to tear down
            each non-None group.  Defaults to ``dist.destroy_process_group``;
            callers that need NCCL-abort-fast-path or a bounded-wait
            destroy should pass their own (e.g. MoEvement's
            ``_abort_or_destroy`` from ``deepspeed.moevement.comm_rebuild``).
            ``None`` groups are skipped without calling ``destroy_fn``.
    """
    global _TENSOR_MODEL_PARALLEL_GROUP, _MODEL_PARALLEL_GROUP, _DATA_PARALLEL_GROUP
    global _WORLD_GROUP, _ZERO_PARAM_INTRA_PARALLEL_GROUP

    if destroy_fn is None:
        destroy_fn = dist.destroy_process_group

    # ``_WORLD_GROUP`` used to hold a ``new_group``-built clone that
    # needed explicit teardown.  Post ``_clone_world_group`` simplification
    # it's just a cached alias for the default WORLD sentinel, so the
    # destroy list skips it — tearing down WORLD here would double-destroy
    # under the survivor-rendezvous path (which owns WORLD teardown
    # explicitly) and is unwanted under the torchrun rebuild path (where
    # WORLD is still live and the caller relies on it).  The cache is
    # still nulled below so the next ``_clone_world_group`` call re-reads
    # the current default (matters across destroy+init cycles).
    singletons = [
    ...
    for pg in singletons:
        if pg is not None:
            destroy_fn(pg)

    for pg in _EXPERT_PARALLEL_GROUP.values():
        if pg is not None:
            destroy_fn(pg)
    for pg in _EXPERT_DATA_PARALLEL_GROUP.values():
        if pg is not None:
            destroy_fn(pg)
    for pg in _ALL_TO_ALL_GROUP.values():
        if pg is not None:
            destroy_fn(pg)
