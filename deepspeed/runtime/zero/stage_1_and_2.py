...
class DeepSpeedZeroOptimizer(ZeROOptimizer):
    ...
    def relink_hp_params(self, new_dp_groups):
        """Repoint every lp_param._dp_group after a comm-group rebuild.

        Spare-rank substitution destroys the original dp groups and
        rebuilds fresh ones.  The lp_params wired up at optimizer init
        still hold the original ProcessGroup via ``_dp_group`` (set by
        :func:`deepspeed.utils.mixed_precision_linkage._init_lp_to_hp_mapping`);
        any subsequent ``safe_get_full_hp_param`` /
        ``safe_set_full_fp32_param`` call issues an all_reduce on the
        torn-down group and deadlocks.

        This method swaps the per-param-group entries in
        ``self.real_dp_process_group`` with the caller-supplied fresh
        groups and re-runs :meth:`_link_all_hp_params` so every
        lp_param picks up the new reference.

        Scoped to 1:1 spare substitution (every new group has the same
        world size as the old one): partition boundaries are unchanged,
        so no re-partition of ``bit16_groups_flat`` /
        ``single_partition_of_fp32_groups`` is needed.  k-of-n survival
        rebuilds would require re-partitioning and are out of scope.

        Does NOT update ``self.dp_process_group`` (consumed by the
        global gradient all-reduce path) or ``self.expert_dp_process_group``
        (consumed by MoE expert grad reduction).  Callers that need
        those refreshed should do so through the engine-level rebuild
        path — this primitive targets only the ``_hp_mapping`` linkage.

        Args:
            new_dp_groups: list of fresh ProcessGroup objects, indexed
                by param group (same order as
                ``self.optimizer.param_groups``).

        Raises:
            ValueError: when the number of new groups doesn't match
                the number of param groups, or when any new group's
                world size differs from the corresponding old group.
        """
        num_groups = len(self.optimizer.param_groups)
        if len(new_dp_groups) != num_groups:
            raise ValueError(f"relink_hp_params: expected {num_groups} new dp_groups "
                             f"(one per param group), got {len(new_dp_groups)}")
        for i, new_group in enumerate(new_dp_groups):
            old_size = dist.get_world_size(group=self.real_dp_process_group[i])
            new_size = dist.get_world_size(group=new_group)
            if new_size != old_size:
                raise ValueError(f"relink_hp_params: dp_group[{i}] world size changed "
                                 f"({old_size} -> {new_size}); re-partition is out of scope.")
        for i, new_group in enumerate(new_dp_groups):
            self.real_dp_process_group[i] = new_group
        self._link_all_hp_params()
    ...
    def relink_all_dp_refs(self,
        ...
        """Repoint every cached DP / expert / MP ProcessGroup reference on the optimizer.

        Extends :meth:`relink_hp_params` (which only covers
        ``real_dp_process_group`` + lp_param linkage) to also swap the
        singular ``dp_process_group`` (consumed by ``allreduce_gradients``
        and grad-norm reduction), ``ep_process_group`` (stored for MoE
        setup), ``expert_dp_process_group`` dict (consumed by MoE
        expert-grad reduction), and ``model_parallel_group`` (consumed
        by ``_model_parallel_all_reduce`` inside the FP16 overflow
        check during ``step``).

        Builds the per-param-group list used by ``relink_hp_params`` the
        same way ``__init__`` does: non-MoE param groups get the fresh
        default DP group; MoE param groups (identified via
        :meth:`is_moe_group`) get the fresh expert-dp group looked up by
        ``group['name']``.  If a MoE param group's name is missing from
        ``new_expert_dp_groups`` the call raises rather than silently
        leaving that param group linked to the old (torn-down) group.

        Scoped to 1:1 spare substitution (every new group has the same
        world size as its old counterpart) — enforced by the inner
        :meth:`relink_hp_params` call.

        Args:
            new_dp_group: fresh default data-parallel process group.
            new_expert_parallel_group: fresh expert-parallel group (for
                MoE all-to-all).  Optional; pass ``None`` for non-MoE
                jobs or when the caller manages this ref separately.
            new_expert_dp_groups: dict ``{group_name: fresh_pg}`` for
                MoE expert-data-parallel groups.  Required iff the
                optimizer has MoE param groups.

        Raises:
            ValueError: when a MoE param group's name isn't in
                ``new_expert_dp_groups``.
        """
        per_pg_list = []
        for group in self.optimizer.param_groups:
            if self.is_moe_group(group):
                name = group['name']
                if new_expert_dp_groups is None or name not in new_expert_dp_groups:
                    raise ValueError(f"relink_all_dp_refs: MoE param group '{name}' has no "
                                     f"corresponding fresh expert-dp group in new_expert_dp_groups; "
                                     f"pass {{'{name}': <fresh_pg>}} or drop the MoE group before rebuild.")
                per_pg_list.append(new_expert_dp_groups[name])
            else:
                per_pg_list.append(new_dp_group)

        self.dp_process_group = new_dp_group
        if new_expert_parallel_group is not None:
            self.ep_process_group = new_expert_parallel_group
        if new_expert_dp_groups is not None:
            self.expert_dp_process_group = dict(new_expert_dp_groups)
        # ``model_parallel_group`` is consumed by
        # ``_model_parallel_all_reduce`` inside the FP16 overflow check
        # on every ``step``.  Leaving it pointing at the destroyed
        # pre-rebuild group makes ``get_backend(pg)`` return None and
        # the very next optimizer step raises ``TypeError: Invariant
        # encountered: value was None when it should not be``.  Only
        # relink when the caller supplies a fresh group — a no-op on
        # non-MP jobs (where the original was already ``None``).
        if new_model_parallel_group is not None:
            self.model_parallel_group = new_model_parallel_group
        self.relink_hp_params(per_pg_list)
    ...
    def step(self, closure=None):
        ...
        # Link the per-param ``_hp_mapping.optim_fragment`` once every
        # group has stepped.  Calling this inside ``_optimizer_step``
        # fired after group 0 only — the gate (``_hp_optimizer_states_linked``)
        # then short-circuited every subsequent invocation, so groups
        # 1..N (e.g. MoE expert groups, which sit after the non-expert
        # group) silently received an empty ``optim_fragment`` because
        # their ``optimizer.state[flat_hp_partition]`` was a defaultdict
        # miss at link time.  Downstream code that walks the mapping
        # (e.g. MoEvement's ``_collect_param_optim_state``) then emitted
        # ``params.*`` only — no Adam moments — for every non-first
        # group's params.
