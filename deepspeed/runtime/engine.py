...
class DeepSpeedEngine(Module):
    ...
    def __init__(self,
                 ...
                 skip_initial_broadcast=False):
        super(DeepSpeedEngine, self).__init__()
        self.dont_change_device = dont_change_device
        # Recovery-mode cold-starts (a spare joining a post-fault world)
        # must skip the initial param broadcast: the spare's weights are
        # fresh random init, but the shared post-rebuild NCCL comm
        # already has live survivors whose pre-fault `_broadcast_model`
        # ran on the *pre-fault* comm.  Dispatching `_broadcast_model`
        # here would both (a) overwrite survivors' trained state with
        # the spare's random init and (b) introduce an op-order
        # asymmetry with survivors on the new comm that blocks all
        # subsequent collectives on the survivor side.  Recovery paths
        # load the spare's params via snapshot/peer-pull instead.
        self._skip_initial_broadcast = skip_initial_broadcast
        ...
        # MoEvement coordinator construction is deferred to here so it
        # can pick up ``self.optimizer.dp_process_group`` — the ZeRO
        # DP group used for peer replication.  The init-time gates
        # (MoE-required, PP>1) already ran in
        # ``_configure_distributed_model`` above.
        self._configure_moevement_coordinator()
    ...
    def _configure_distributed_model(self, model):
        ...
        # Initialize MoEvement sparse checkpointing if enabled.  MoEvement's
        # whole design assumes the per-iteration work can be bucketed into
        # "operators" that are individually snapshotable — experts, gating,
        # non-expert.  A dense transformer has no such decomposition, and
        # every operator-granularity optimization collapses to "snapshot
        # everything every window", which defeats the sparse checkpointing
        # purpose.  Fail loudly if the user enabled MoEvement on a model
        # without MoE layers instead of silently no-op'ing the feature.
        if self._config.moevement_config.enabled and not self.has_moe_layers:
            raise ValueError("MoEvement is only supported for models with MoE layers, but "
                             "no MoELayer was discovered in the module.  Either disable "
                             "moevement in the DeepSpeed config or use a model that "
                             "defines ``deepspeed.moe.layer.MoELayer`` submodules.")
        # MoEvement's replay semantics depend on inter-stage activation
        # and gradient logs captured at pipeline boundaries.  Under PP=1
        # (non-pipeline training, e.g. MoE + DP + EP only) there are no
        # pipeline boundaries, so the replay has nothing to drive it
        # and recovery collapses to "reload the full checkpoint".
        # Require ``PipelineEngine`` explicitly at init time rather than
        # silently no-op'ing on recovery later.  Deferred import
        # sidesteps the circular dependency between ``runtime.engine``
        # and ``runtime.pipe.engine``.
        if self._config.moevement_config.enabled:
            from deepspeed.runtime.pipe.engine import PipelineEngine
            if not isinstance(self, PipelineEngine):
                raise ValueError("MoEvement requires pipeline parallelism (PP > 1); its replay "
                                 "mechanism reconstructs state from inter-stage activation and "
                                 "gradient logs that only exist under a pipeline schedule.  "
                                 "Either disable moevement in the DeepSpeed config, or wrap the "
                                 "model in ``deepspeed.pipe.PipelineModule`` and call "
                                 "``deepspeed.initialize`` with a pipeline config whose number "
                                 "of stages is greater than 1.")
        # ZeRO stages 2 and 3 partition gradients across DP peers, which
        # requires the backward pass to reduce-scatter grads into a flat
        # buffer.  ``PipelineEngine`` instead schedules gradient all-
        # reduces itself at pipeline boundaries (it disables
        # ``enable_backward_allreduce`` in ``super().backward()``), so
        # the two gradient-flow strategies are mutually exclusive; the
        # pipe engine rejects stage >= 2 up-front with a generic
        # assertion.  Because moevement mandates PP > 1, the transitive
        # implication is that moevement + stage >= 2 can never work.
        # Catch it here with a MoEvement-specific error so the user
        # sees which feature in their config caused the conflict
        # before the downstream pipe-engine assertion fires.
        if self._config.moevement_config.enabled and self.zero_optimization_stage() >= ZeroStageEnum.gradients:
            raise ValueError("MoEvement is incompatible with ZeRO-2 and ZeRO-3; it requires "
                             "pipeline parallelism, and ``PipelineEngine`` schedules gradient "
                             "all-reduces itself at pipeline boundaries, so partitioned-"
                             "gradient ZeRO stages cannot coexist.  Use ZeRO stage 0 or 1, "
                             "or disable moevement in the DeepSpeed config.")
        # Coordinator construction + ``set_topology`` + ``initialize``
        # runs from ``_configure_moevement_coordinator`` after
        # ``_configure_optimizer`` so that ``self.optimizer`` exists
        # and exposes ``dp_process_group``.  If we did it here the
        # optimizer wouldn't be built yet — ``self.optimizer`` would
        # still be ``None`` — and every replication collective would
        # silently fall back to world-rank defaults and no-op.
        ...
        if not (self.amp_enabled() or is_zero_init_model or self._skip_initial_broadcast):
            self._broadcast_model()

    def _configure_moevement_coordinator(self):
        """Build the MoEvement coordinator once the optimizer is ready.

        The init-time gates (MoE layers present, PP > 1) already fired
        in ``_configure_distributed_model``.  Construction itself is
        deferred until after ``_configure_optimizer`` has populated
        ``self.optimizer`` because the coordinator needs ZeRO's
        ``dp_process_group`` for its peer-replication ring — without
        it, ``_build_gloo_mirror(None)`` returns ``None`` and the whole
        replication path no-ops, which also defeats any peer-pull
        recovery that relies on ring replicas.
        """
        if not (self.has_moe_layers and self._config.moevement_config.enabled):
            return
        ...
        _dp_pg = getattr(self.optimizer, 'dp_process_group', None) if self.optimizer is not None else None
        _dp_rank = dist.get_rank(group=_dp_pg) if _dp_pg is not None else dist.get_rank()
        self.moevement_coordinator.set_topology(
            dp_group=_dp_pg,
            dp_rank=_dp_rank,
            device=self.device,
        )
        # The gates above guarantee ``self.module`` is a
        # ``PipelineModule``, so ``_grid`` is always present.  Pulling
        # the stage id here (rather than waiting for
        # ``PipelineEngine.__init__``'s later ``set_pipeline_topology``)
        # lets ``_discover_operators`` stage-qualify operator names on
        # the first pass — otherwise it runs with ``_stage_id=None`` and
        # emits colliding ``layer_0_*`` labels on every pipeline stage.
        stage_id = self.module._grid.get_stage_id()
        self.moevement_coordinator.initialize(
            model=self.module,
            moe_layers=self.moe_layers,
            iter_time_sec=self._config.moevement_config.initial_iter_time_sec,
            stage_id=stage_id,
    ...
    # ------------------------------------------------------------------
    # Communication-group rebuild (Layer 2.B of DESIGN_COMM_GROUP_REBUILD)
    # ------------------------------------------------------------------

    def rebuild_nccl_groups(self, world_rendezvous_fn=None):
        """Rebuild every cached process group after a spare-rank substitution.

        Called by the cluster / launcher after
        ``dist.init_process_group`` has been re-invoked to bring the
        spare rank into the world.  Composes the Layer 2.A primitives
        (``groups.reset_for_rebuild`` +
        ``optimizer.relink_all_dp_refs``) with engine-level re-init so
        every cached ProcessGroup reference — in the
        ``deepspeed.utils.groups`` module globals, on the engine
        instance, on the optimizer, and on every ZeRO-managed lp_param
        — ends up pointing at a freshly-constructed communicator.
        Then delegates to Layer 1 via ``moevement_coordinator.rebuild_comm_groups``
        to rebuild the gloo mirrors.

        Ordering (pinned by
        ``tests/unit/runtime/test_engine_rebuild.py``):

        1. Quiesce MoEvement's in-flight replication + disk-persist so
           nothing holds a reference to the groups about to be aborted.
        2. ``groups.reset_for_rebuild`` with MoEvement's
           ``_abort_or_destroy`` as the destroy callable so wedged
           NCCL collectives are aborted rather than destroy-blocking.
        2.5. **(Optional, if ``world_rendezvous_fn`` is provided.)**
           Call the rendezvous callable.  The survivor-preservation
           path (see ``survivor_rendezvous``) uses this hook to
           destroy the old default PG and re-invoke
           ``init_process_group`` against a new master
           addr/port (the old rank-0 may be the rank that died, so
           the old master is gone).  Must run after step 2 so the
           sub-group backend teardown talks to a still-valid WORLD,
           and before step 3 so the sub-group rebuild binds to the
           new WORLD.  Default ``None`` preserves the
           spare-joined-via-launcher flow where WORLD has already
           been re-inited externally.
        3. Rebuild the pipe-grid's sub-groups
           (``_rebuild_pipe_sub_groups``) — no-op in the base engine;
           ``PipelineEngine`` overrides to tear down and re-create its
           four sub-group families.  Runs BEFORE the MoE re-init in
           step 4 so the ``dist.new_group`` call order on survivors
           matches the cold-start order on a spare rank (which
           builds grid sub-groups via ``PipelineModule`` before
           ``deepspeed.initialize`` triggers MoE
           ``set_deepspeed_parallelism``).  NCCL derives its store-
           key prefix from the per-PG creation counter, so any
           spare-vs-survivor drift in ``new_group`` order makes
           counter N point at different semantic groups across ranks
           and lazy NCCL bootstrap fails with ``store->get('0')``.
        4. Re-run the group-construction portion of first-time init
           (``_reinitialize_distributed_groups``) so the freshly
           cleared module globals (MoE expert-parallel + expert-
           data-parallel groups) get repopulated against the new
           world.
        5. Refresh the engine-cached attrs
           (``_refresh_group_caches``) by re-reading ``groups._get_*``.
        6. Relink the optimizer's cached DP-group references + every
           ZeRO ``lp_param._dp_group`` via ``relink_all_dp_refs``.
        7. Rebuild the MoEvement gloo mirrors (Layer 1).

        Scoped to 1:1 spare substitution: every new group has the same
        world size as its old counterpart.  k-of-n rebuilds would
        require re-partitioning ZeRO's bit16/fp32 groups, and a
        mesh-device-based init would need the mesh re-built externally
        — both out of scope for this rebuild path (see design §4).

        Args:
            world_rendezvous_fn: Optional callable, no args.  When
                supplied, invoked between step 2 and step 3 — use
                this to destroy + re-invoke ``init_process_group``
                inside the rebuild sequence for the survivor-
                preservation path.  See ``survivor_rendezvous``.

        Must be invoked by every rank in lockstep.  No-op on
        single-rank jobs; callers are responsible for the prerequisite
        re-invocation of ``dist.init_process_group`` on the fresh
        world before calling this method (or for passing a
        ``world_rendezvous_fn`` that does so).
        """
        # Deferred import: ``deepspeed.moevement.comm_rebuild`` imports
        # from ``deepspeed.utils.logger`` which is already loaded by
        # the time ``rebuild_nccl_groups`` can be invoked.  Keeping the
        # import local avoids pulling MoEvement into stock DeepSpeed's
        # module-load graph on pure non-MoEvement runs.
        from deepspeed.moevement.comm_rebuild import _abort_or_destroy

        coord = getattr(self, 'moevement_coordinator', None)

        # 1. Quiesce in-flight MoEvement ops.  Running before abort is
        #    load-bearing: the replication worker thread holds a
        #    reference to the old gloo mirror and a mid-flight send on
        #    an aborted communicator is undefined behaviour.
        if coord is not None:
            coord._quiesce_pending_replication_and_persist(coord.config.comm_rebuild_timeout_sec)

        # 2. Abort + clear every cached group.  ``_abort_or_destroy``
        #    prefers NCCL ``abort()`` when available so a wedged
        #    communicator (dead rank left an in-flight op) tears down
        #    cleanly instead of blocking destroy forever.
        groups.reset_for_rebuild(destroy_fn=_abort_or_destroy)

        # 2.5. If the caller supplied a world-rendezvous hook, run it
        #      here — between sub-group teardown (step 2) and sub-group
        #      rebuild (step 3).  The survivor-preservation flow uses
        #      this hook to destroy the old default PG and re-invoke
        #      ``init_process_group`` against a new master before the
        #      fresh sub-groups are built.
        if world_rendezvous_fn is not None:
            world_rendezvous_fn()

        # 3. Rebuild the pipe-grid sub-groups FIRST so the
        #    ``dist.new_group`` call order on survivors matches the
        #    cold-start order on a spare rank (which builds grid
        #    sub-groups via ``PipelineModule`` before ``deepspeed.initialize``
        #    triggers MoE ``set_deepspeed_parallelism``).  NCCL derives
        #    its store-key prefix from the per-PG creation counter, so
        #    any spare-vs-survivor drift in ``new_group`` order makes
        #    counter N point at different semantic groups across ranks
        #    → lazy NCCL bootstrap fails with ``store->get('0')`` on
        #    the first collective.  No-op in the base engine.
        self._rebuild_pipe_sub_groups()

        # 4. Repopulate the groups module globals (MoE expert-parallel
        #    + expert-data-parallel groups) against the fresh world.
        #    Runs after the grid rebuild so the MoE ``new_group`` calls
        #    land at the same counter slots they did on the spare's
        #    cold-start path.
        self._reinitialize_distributed_groups()

        # 5. Re-read the fresh groups into engine-cached attrs.
        self._refresh_group_caches()

        # 6. Point the optimizer's cached DP refs + every lp_param
        #    ``_dp_group`` at the new groups.  Gracefully skip on
        #    optimizer types that don't implement the relink
        #    primitive (bf16 optimizer, ZeRO-3) — the caller can
        #    rebuild the optimizer themselves for those paths.
        if self.optimizer is not None and hasattr(self.optimizer, 'relink_all_dp_refs'):
            new_ep_dict = self.expert_parallel_group if self.has_moe_layers else None
            new_edp_dict = self.expert_data_parallel_group if self.has_moe_layers else None
            # Fresh model-parallel group for ZeRO's FP16-overflow-check
            # allreduce.  ``self.mpu`` is the PipelineParallelGrid on
            # PipelineEngine (rebuilt in step 4) or the ``groups`` module
            # on plain DeepSpeedEngine (where ``get_model_parallel_group``
            # resolves against the freshly-refreshed caches from step 5).
            # Absent an ``mpu``, leave ``new_mp_group`` None so the
            # optimizer keeps its original ``None`` — nothing to relink.
            new_mp_group = None
            mpu = getattr(self, 'mpu', None)
            if mpu is not None and hasattr(mpu, 'get_model_parallel_group'):
                new_mp_group = mpu.get_model_parallel_group()
            self.optimizer.relink_all_dp_refs(
                new_dp_group=self.seq_data_parallel_group,
                new_expert_parallel_group=new_ep_dict,
                new_expert_dp_groups=new_edp_dict,
                new_model_parallel_group=new_mp_group,
            )
        elif self.optimizer is not None:
            logger.warning(
                "[rebuild_nccl_groups] optimizer %s has no relink_all_dp_refs; "
                "cached ProcessGroup references in the optimizer may be stale.  "
                "Only ZeRO stage 0/1 is supported by the rebuild path today.",
                type(self.optimizer).__name__)

        # 7. Layer 1: MoEvement gloo mirror rebuild against the
        #    freshly-rebuilt dp + pp base groups (the pipe-grid
        #    sub-groups were rebuilt in step 4, so
        #    ``_rebuild_get_pipeline_parallel_group`` returns the fresh
        #    pp group here).
        if coord is not None:
            new_pp = self._rebuild_get_pipeline_parallel_group()
            coord.rebuild_comm_groups(new_dp_group=self.seq_data_parallel_group, new_pp_group=new_pp)

    def _rebuild_pipe_sub_groups(self):
        """Hook for pipe-grid sub-group rebuild; no-op in the base engine.

        Overridden by ``PipelineEngine`` to tear down and re-create its
        four sub-group families (``ds_model_proc_group``,
        ``dp_proc_group``, ``pp_proc_group``, ``slice_proc_group``).
        Sequenced before ``_reinitialize_distributed_groups`` in
        ``rebuild_nccl_groups`` so the ``dist.new_group`` call order
        on survivors matches the cold-start order on a spare rank
        (grid-then-MoE).  Running it after MoE re-init would leave
        NCCL store-key prefixes misaligned across ranks (counter N
        points at different semantic groups on spare vs survivors)
        and the first post-rebuild collective would fail with
        ``store->get('0') got error``.
        """
        pass

    def _reinitialize_distributed_groups(self):
        """Re-run the group-construction half of first-time init.

        Mirrors the MoE-parallelism pass + ``mpu`` propagation from
        ``_configure_distributed_model`` (the lines that fire
        ``groups._create_expert_and_data_parallel`` /
        ``_create_expert_data_and_model_parallel`` via each MoE
        layer's ``set_deepspeed_parallelism``).  Everything else
        (``_clone_world_group``, ``_get_data_parallel_group``) is
        lazy — the ``is None`` / dict-membership short-circuits flip
        back to the build path after ``reset_for_rebuild`` nulls the
        globals, so a later ``_get_*`` call rebuilds on demand.

        Before re-invoking ``set_deepspeed_parallelism``, nulls every
        MoELayer's cached ``ep_group`` — ``MOELayer._set_ep_group``
        asserts the slot is ``None`` as a one-time-init guard, so a
        naive re-entry would hit that assert after first init
        already populated the group.  Resetting the ref is safe
        because the old group was destroyed by ``reset_for_rebuild``
        and a fresh one gets assigned inside ``_create_process_groups``.
        """
        if self.mpu is not None:
            groups.mpu = self.mpu

        # Clear the one-time-init guard in every MoELayer so
        # ``set_deepspeed_parallelism`` can re-enter.  ``MOELayer._set_ep_group``
        # writes both ``self.ep_group`` and ``self.gate.ep_group`` — the
        # gate-level assert is the one that actually fires on re-entry
        # (the MOELayer-level slot is unconditionally overwritten), so
        # clearing both is load-bearing.
        for _, module in self.module.named_modules():
            inner = getattr(module, "deepspeed_moe", None)
            if inner is not None:
                if hasattr(inner, "ep_group"):
                    inner.ep_group = None
                gate = getattr(inner, "gate", None)
                if gate is not None and hasattr(gate, "ep_group"):
                    gate.ep_group = None
    ...
    def _refresh_group_caches(self):
        """Re-read ``groups._get_*`` into engine-cached attrs.

        Each of the attrs below holds a direct ProcessGroup reference
        that was captured in ``_configure_distributed_model`` at init
        time; after rebuild the underlying module-global has flipped to
        a fresh group, but the engine attr still points at the old one.
        This helper repoints them by re-reading the accessors.
        """
        self.local_all_to_all_group = None
        if self.zero_quantized_gradients():
            self.local_all_to_all_group = groups._get_local_all_to_all_group()
    ...
    def _rebuild_get_pipeline_parallel_group(self):
        """Return the PP group to feed into the MoEvement Layer-1 rebuild.

        Base ``DeepSpeedEngine`` has no pipeline-parallel topology, so
        there's nothing to supply.  ``PipelineEngine`` overrides this
        to return the freshly rebuilt ``self.grid.get_pipe_parallel_group()``.
        """
        return None

    def survivor_rendezvous(self, new_master_addr, new_master_port, new_rank=None, new_world_size=None):
        """Re-init the default PG on a new master and rebuild every cached group.

        Called on surviving ranks after a peer has died, once the
        launcher / supervisor has decided on the new world layout and
        notified every survivor of the new master endpoint.  Composes
        the full 8-step sub-group rebuild (``rebuild_nccl_groups``)
        with an inner ``destroy_process_group`` +
        ``init_process_group`` step that swaps the default PG onto
        the new master.  The old master may have been the rank that
        died, so ``new_master_addr`` / ``new_master_port`` are
        required — the agent is responsible for ensuring every
        surviving rank + the replacement spare sees the same values.

        Survivors stay in the same Python process throughout: no
        interpreter re-init, no CUDA context tear-down, no model
        reconstruction.  The only cost on the survivor is the
        destroy / re-init round-trip plus the usual sub-group rebuild
        — orders of magnitude cheaper than a whole-job restart.

        Scoped to 1:1 spare substitution (same topology, same world
        size, just a new master endpoint + possibly new rank
        assignment).  Non-1:1 rebuilds are out of scope for the same
        reasons ``rebuild_nccl_groups`` documents.

        **Optimizer compatibility.**  Verified with vanilla torch Adam +
        ZeRO-1.  Known latent bug with 1bit-Adam / 1bit-Lamb /
        ZeroOneAdam: ``runtime/comm/{nccl,compressed,hccl}.py`` build
        their own ``self.world_group`` at init via
        ``dist.new_group(ranks=range(world_size))``, the same pattern
        Layer 1 / Layer 4 eliminated elsewhere.  A spare joining a
        post-fault world under 1bit optimizers will hit the familiar
        ``store->get('0') got error`` counter-drift on the first
        post-rebuild collective on those helpers.  Fix is the same as
        Layer 4 (switch to ``dist.get_world_group()``) but unvalidated;
        MoEvement does not ship with 1bit optimizers, so this is
        documented rather than fixed.  See
        ``docs/moevement/HISTORY.md#layer-3-investigation`` §12 for the audit.

        Args:
            new_master_addr: Host the post-fault rank 0 will bind to.
            new_master_port: Port the post-fault rank 0 will bind to.
            new_rank: This rank's rank in the new world.  Default:
                current rank (unchanged under 1:1 substitution).
            new_world_size: Size of the new world.  Default: current
                world size (unchanged under 1:1 substitution).

        Must be invoked by every surviving rank in lockstep with the
        replacement spare's own ``init_process_group`` call — they
        rendezvous together on the new master.
        """
        import os
        from deepspeed.accelerator import get_accelerator
        if new_rank is None:
        ...
        def _rendezvous():
            # Drop the default PG so the subsequent init call re-binds
            # against the new master without a "already initialized"
            # guard tripping.  Sub-groups have already been aborted +
            # cleared by ``groups.reset_for_rebuild`` (step 2 of the
            # surrounding ``rebuild_nccl_groups``), so this only
            # disposes of the WORLD group itself.
            dist.destroy_process_group()

            # ``init_distributed`` reads master addr / port / rank /
            # world-size from the environment by default; set them so
            # every process bootstraps against the supervisor-chosen
            # endpoint.  ``dist_init_required=True`` forces a re-init
            # path even though ``cdb`` is still populated from the
            # original init.
            os.environ["MASTER_ADDR"] = str(new_master_addr)
            os.environ["MASTER_PORT"] = str(new_master_port)
            os.environ["RANK"] = str(new_rank)
            os.environ["WORLD_SIZE"] = str(new_world_size)
            # LOCAL_RANK typically tracks RANK on single-node runs;
            # keep it in sync so the TorchBackend init picks the
            # right GPU for its ``device_id`` hint.
            os.environ["LOCAL_RANK"] = str(new_rank)
            dist.init_distributed(dist_backend=backend,
                                  auto_mpi_discovery=False,
                                  dist_init_required=True,
                                  rank=new_rank,
                                  world_size=new_world_size)

        self.rebuild_nccl_groups(world_rendezvous_fn=_rendezvous)

        # Under 1:1 substitution these don't actually change, but re-
        # reading them is cheap and keeps the engine's cached rank +
        # world-size in sync with the default PG if a caller ever
        # uses ``survivor_rendezvous`` for a non-trivial world swap.
        self.global_rank = dist.get_rank()
        self.world_size = dist.get_world_size()
    ...
    def _take_model_step(self, lr_kwargs, block_eigenvalue={}):
        ...
        # MoEvement recovery: replay re-runs past training steps, so all
        # per-iteration counters and schedulers must be pinned.  Pinning
        # ``global_steps`` keeps the recovering rank in lockstep with
        # paused DP peers (who don't iterate); pinning the LR and
        # compression schedulers keeps the update magnitudes matched
        # between recovering and cascade-triggered peers.  Without
        # this, replay would advance scheduled LRs W_sparse ticks while
        # paused peers stay put, breaking DP averaging on resume.
        is_moevement_replaying = (self.moevement_coordinator is not None
                                  and self.moevement_coordinator.is_recovering())

        if overflow:
            self.skipped_steps += 1
        elif not is_moevement_replaying:
    ...
    def step(self, lr_kwargs=None):
        ...
        if self.is_gradient_accumulation_boundary():
            ...
            # MoEvement: queue the cross-stream wait_event for the prior
            # boundary's snapshot D2H (so optim.step doesn't write the
            # master / optim state the side-stream copy is still reading)
            # and, in recovery, zero weight gradients on frozen operators
            # so ZeRO's gradient partition contains zeros and the Adam
            # update leaves their weights unchanged during replay.  Must
            # fire every iter — gating on ``is_recovering()`` left the
            # snapshot D2H without a main-stream consumer wait, voiding
            # the ordering guarantee the deferred-sync path relies on.
            # The recovery-specific ``zero_frozen_gradients`` is gated
            # inside the coordinator method itself.
            if self.moevement_coordinator is not None:
                self.moevement_coordinator.on_before_optimizer_step(self.module)
        ...
        if self.moevement_coordinator is not None and self.is_gradient_accumulation_boundary():
            # Hand per-iter engine scalars into the coordinator so disk
            # saves + peer-pull responses can ship fault-time counters +
            # scheduler state to a recovering rank.  These values don't
            # advance during replay, so the spare needs them restored
            # directly (scaler, in contrast, is captured at finalize_window
            # and replay advances it iter-for-iter).  See
            # ``coordinator.capture_engine_scalars``.
            engine_scalars = self.moevement_coordinator.capture_engine_scalars(self)
            self.moevement_coordinator.on_iteration_end(self.global_steps,
                                                        self.module,
                                                        self.optimizer,
                                                        engine_scalars=engine_scalars)
    ...
    def load_checkpoint(self,
        ...
        if self.moevement_coordinator is not None and load_path is not None:
            self.moevement_coordinator.load_sparse_checkpoint(load_dir, tag, model=self.module, engine=self)
    ...
    def save_checkpoint(self, save_dir, tag=None, client_state={}, save_latest=True, exclude_frozen_parameters=False):
        ...
        # Gated on the moevement.persist_to_disk config knob (default False):
        # peer-pull from a surviving DP peer's pinned host snapshot is the
        # primary recovery target, so the disk bundle is opt-in.  Setting the
        # knob to True restores the historical Whole-Cluster Restart path —
        # see resume_after_fault.py.
        if (self.moevement_coordinator is not None
                and getattr(self.moevement_coordinator.config, "persist_to_disk", False)):
    ...
    def moevement_trigger_recovery(self, failed_stage_id=None):
        """Explicitly enter MoEvement recovery mode.

        Intended for orchestration layers (k8s operators, SLURM epilogs,
        user watchdogs) that detect a pipeline-stage failure out of band
        and want the replacement rank to pull replay logs from its
        neighbours instead of starting fresh.  The whole-job restart
        path triggers recovery automatically from load_checkpoint, so
        users of that path do not need to call this.

        Args:
            failed_stage_id: Pipeline stage that failed (None for non-pipeline).
        """
        if self.moevement_coordinator is not None:
            self.moevement_coordinator.begin_recovery(failed_stage_id=failed_stage_id)
