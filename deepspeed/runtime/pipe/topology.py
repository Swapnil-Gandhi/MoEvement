...
class PipelineParallelGrid:
    ...
    def _build_sub_groups(self):
        """Build (or rebuild) the four sub-group families this grid owns.

        Factored out of ``__init__`` so the same construction logic can
        be re-run during a comm-group rebuild after a spare-rank
        substitution (see ``rebuild_sub_groups``).  Each call issues
        fresh ``dist.new_group`` calls — callers that need to discard
        a previous set of groups must tear them down first via
        ``_abort_sub_groups`` or the rebuild helper, otherwise the old
        communicators leak.
        """
    ...
    def _abort_sub_groups(self, destroy_fn):
        """Tear down the current sub-group set via ``destroy_fn``.

        Covers the four group families ``_build_sub_groups`` populates:
        ``ds_model_proc_group``, ``dp_proc_group``, ``pp_proc_group``,
        and ``slice_proc_group``.  ``destroy_fn`` is the caller-supplied
        teardown callable (e.g. MoEvement's ``_abort_or_destroy`` for
        NCCL-abort-fast-path on torch >= 2.6).  Attributes that were
        never populated (e.g. when the rebuild fires before first-time
        init completed) are tolerated — we check ``hasattr`` rather
        than assuming the grid is fully initialised.
        """
        for attr in ("ds_model_proc_group", "dp_proc_group", "pp_proc_group", "slice_proc_group"):
            pg = getattr(self, attr, None)
            if pg is not None:
                destroy_fn(pg)
                setattr(self, attr, None)

    def rebuild_sub_groups(self, destroy_fn):
        """Re-create the four sub-group families after a world-topology change.

        Called from ``PipelineEngine.rebuild_nccl_groups`` during a
        spare-rank substitution.  Aborts the current set via
        ``destroy_fn``, then re-runs ``_build_sub_groups`` to populate
        a fresh set against whatever process groups
        ``deepspeed.comm`` now wraps.

        Must be invoked by every rank in lockstep — ``_build_sub_groups``
        issues collective ``dist.new_group`` calls and those have
        ordering constraints across ranks.  The caller must have
        already re-run the stock ``groups.reset_for_rebuild`` +
        ``_reinitialize_distributed_groups`` sequence on the engine
        so the base groups the pipe grid sits on top of are themselves
        fresh.
        """
        self._abort_sub_groups(destroy_fn)
        self._build_sub_groups()
