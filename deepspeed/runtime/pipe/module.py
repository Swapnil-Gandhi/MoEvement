...
class PipelineModule(nn.Module):
    ...
    def __init__(self,
        ...
        # Setup world info.  Historically this was
        # ``dist.new_group(ranks=range(dist.get_world_size()))`` to get
        # a PG distinct from ``dist.group.WORLD``, but the ``new_group``
        # call consumes a store-keyed counter on the default PG's
        # TCPStore.  Under a survivor-rendezvous comm rebuild, the
        # survivor-rebuild path (``_rebuild_pipe_sub_groups``) does NOT
        # re-issue this world-clone call (the rebuild reuses the default
        # PG), while a cold-started spare DOES issue it during its own
        # ``PipelineModule`` construction — the 1-call drift leaves
        # lazy NCCL init on the default PG reading a stale store key
        # and every subsequent WORLD collective on survivors fails with
        # ``store->get('0') got error``.  ``dist.get_world_group()``
        # returns the ``dist.group.WORLD`` sentinel (no ``new_group``,
        # no counter), matches the Layer 1 fix applied to
        # ``_clone_world_group``, and resolves to the post-rebuild
        # default PG correctly after ``destroy_process_group`` + re-init.
