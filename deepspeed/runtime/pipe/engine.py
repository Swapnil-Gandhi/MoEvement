...
class PipelineEngine(DeepSpeedEngine):
    ...
    def __init__(self, has_bool_tensors=False, *super_args, **super_kwargs):
        ...
        # MoEvement: register pipeline topology for localized recovery
        if self.moevement_coordinator is not None:
            self.moevement_coordinator.set_pipeline_topology(
                pp_group=self.grid.get_pipe_parallel_group(),
                stage_id=self.stage_id,
                num_stages=self.num_stages,
                stage_to_global_fn=self.grid.stage_to_global,
            )
        ...
        # The model-parallel allreduce aggregates per-stage param counts
        # into TOTAL_PARAMS / UNIQUE_PARAMS figures used only for the
        # init-time logging line below.  Under ``skip_initial_broadcast``
        # (a spare joining a post-fault world), survivors already built
        # their engines pre-fault and will NOT re-dispatch this collective
        # on the rebuilt ``model_parallel_group``; if the spare dispatches
        # it, the shared group stalls waiting for the missing partner.
        # Fall back to per-stage numbers on the spare path — logging
        # accuracy is the only cost, correctness is preserved.
        if self._skip_initial_broadcast:
        ...
        # Under ``skip_initial_broadcast`` (a spare joining a post-fault
        # world), survivors already ran this adjacent-stage handshake
        # pre-fault on the OLD pipe-parallel group and do NOT re-run it
        # on the rebuilt pp group.  If the spare dispatches these p2p
        # sends/recvs on the new pp group, the matching partner never
        # comes.  Skipping the handshake on the recovery-mode spare
        # keeps op order aligned across survivors + spare; subsequent
        # real pp sends/recvs (from ``train_batch``) will be dispatched
        # symmetrically on every rank.
        if not self._skip_initial_broadcast:
    ...
    def _rebuild_pipe_sub_groups(self):
        """Rebuild the pipe-grid's four sub-group families.

        Sequenced by ``DeepSpeedEngine.rebuild_nccl_groups`` BEFORE
        ``_reinitialize_distributed_groups`` (MoE) so the
        ``dist.new_group`` call order on survivors matches the cold-
        start order on a spare rank (grid-then-MoE).  Any drift in
        order makes NCCL's store-key prefix — derived from the per-PG
        creation counter — point at different semantic groups on
        spare vs survivors, and the first post-rebuild collective
        fails with ``store->get('0') got error``.
        """
        from deepspeed.moevement.comm_rebuild import _abort_or_destroy
        self.grid.rebuild_sub_groups(_abort_or_destroy)

    def _rebuild_get_pipeline_parallel_group(self):
        """Return the fresh pp group for the Layer-1 gloo-mirror rebuild.

        By the time this hook is called from
        ``DeepSpeedEngine.rebuild_nccl_groups`` (step 7 — coordinator
        rebuild), the pipe-grid sub-group rebuild in step 4 has
        already replaced ``self.grid.pp_proc_group``, so this returns
        the freshly-built pp group.
        """
        return self.grid.get_pipe_parallel_group()
    ...
    def train_batch(self, data_iter=None):
        ...
        # MoEvement: all pipeline ranks exchange recovery status so that any
        # stage that called begin_recovery() receives replay logs from its
        # neighbours before the schedule below starts blocking on P2P recvs.
        # ``"abandon"`` signals the rank paused through a cascade and must
        # skip the rest of train_batch — its pp column's broadcast and DP
        # all-reduce have no aligned partners because the recovering ranks
        # are mid-replay, not at the same loss-aggregation point.
        barrier_status = "continue"
        if self.moevement_coordinator is not None:
            barrier_status = self.moevement_coordinator.recovery_barrier(model=self.module)

        if barrier_status == "abandon":
            self.agg_train_loss = torch.zeros((), device=self.device)
            self.agg_additional_losses = None
        ...
        # MoEvement sparse-snapshot trigger.  ``DeepSpeedEngine.step`` fires
        # ``on_iteration_end`` at engine.py:2896, but ``train_batch`` runs
        # its own schedule and never calls the parent ``step``, so without
        # this call snapshotting silently no-ops in MoEvement's supported
        # topology (PP > 1).  Placed after ``_exec_schedule`` — not inside
        # ``_exec_optimizer_step`` — so any in-flight pipeline P2P has
        # drained before the coordinator issues its own DP all-reduces
        # for FP32 param / optimizer-state reassembly.
        if self.moevement_coordinator is not None:
            # Hand fault-time engine scalars into the coordinator so disk
            # saves + peer-pull responses can ship them to a recovering
            # rank.  See the analogous DeepSpeedEngine.step call site for
            # rationale; threading here is mandatory for pipeline engines
            # because they never call the parent ``step``.
    ...
    def _aggregate_total_loss(self):
        ...
        # During MoEvement recovery the downstream pp ranks may be paused in
        # ``recovery_barrier`` and won't join the pp-wide broadcast below;
        # short-circuit to a dummy scalar so the recovering pp column (which
        # only needs gradients, not the aggregated loss, to replay correctly)
        # doesn't deadlock.  The loss value is consumed only by steps_per_print
        # logging, which is fine to lose on recovery iters.  Also short-
        # circuit on the recovering rank's last replay iter — ``end_recovery``
        # already flipped ``_recovering=False`` by then, but paused peers
        # have just exited their wait loop and will ``"abandon"`` this same
        # train_batch, so the pp broadcast still has no partner.
        coord = self.moevement_coordinator
        if coord is not None and (coord.is_recovering() or coord._post_recovery_exit):
            dummy = torch.zeros((), device=self.device)
            self.agg_additional_losses = None
            return dummy
        if self.is_last_stage():
    ...
    def _exec_send_activations(self, buffer_id):
        ...
        # MoEvement recovery: skip the p2p send when our downstream
        # stage isn't going to recv (paused rank).  If downstream is
        # ALSO recovering, send normally — the pipeline-chain replay
        # for contiguous failed stages relies on this: stage N+1 falls
        # through to p2p.recv whenever it has no log from an upstream
        # live stage, and our replay output is exactly what it needs.
        is_recovery = (self.moevement_coordinator is not None and self.moevement_coordinator.is_recovering())
        if is_recovery and self.moevement_coordinator.should_skip_pipeline_send(self.next_stage):
            return

        # MoEvement upstream logging: log activations at stage boundaries.
        # Gated on NOT is_recovery so replay iterations don't re-log the
        # same activations (the pre-failure capture is what recovery
        # actually needs; re-logging during replay just wastes memory).
        if self.moevement_coordinator is not None and not is_recovery:
    ...
    def _exec_send_grads(self, buffer_id):
        ...
        # MoEvement recovery: skip the p2p send when our upstream stage
        # won't recv (paused rank).  If upstream is ALSO recovering —
        # e.g., under contiguous multi-stage failure — send normally so
        # the backward pipeline chains through the failed region via
        # the same p2p channels normal training uses.  Free the input
        # buffer in the skipped case to match the non-recovery cleanup.
        is_recovery = (self.moevement_coordinator is not None and self.moevement_coordinator.is_recovering())
        if is_recovery and self.moevement_coordinator.should_skip_pipeline_send(self.prev_stage):
        ...
        # Placed *after* partitioning + mask-drop so the log captures the
        # exact tensors that hit the wire.  Under is_grad_partitioned the
        # payload is (meta_long, data_fp) — the PartitionedTensor view of
        # the pre-partition fp gradient — and the recovery replay has to
        # install that same shape into grad_layer for the downstream
        # ``PartitionedTensor.from_meta`` reconstruction to succeed.
        # Gated on NOT is_recovery so replay iterations don't re-log —
        # pre-failure captures are what recovery needs.
        if self.moevement_coordinator is not None and not is_recovery:
            if isinstance(inputs, torch.Tensor):
                grads_to_log = inputs.grad
            elif isinstance(inputs, tuple):
                if self.is_grad_partitioned:
                    # The tensors in positions 0 and 1 (meta + data) are
                    # themselves the payload — not ``.grad`` of a tensor.
                    # Tail members have requires_grad=False (enforced by
                    # the forward-pass assert) so nothing else travels.
                    grads_to_log = (inputs[0], inputs[1])
                else:
                    grads_to_log = tuple(elt.grad for elt in inputs
                                         if torch.is_tensor(elt) and elt.is_floating_point())
            else:
                grads_to_log = None
            self.moevement_coordinator.on_send_gradients(grads_to_log, self.global_steps, buffer_id, self.stage_id)
    ...
    def _exec_recv_activations(self, buffer_id):
        # MoEvement recovery: use logged activations instead of blocking P2P receive
        if self.moevement_coordinator is not None and self.moevement_coordinator.is_recovering():
            acts = self.moevement_coordinator.get_replay_activations(self.global_steps, buffer_id)
            if acts is not None:
                recvd = tuple(t.to(self.device).detach().requires_grad_(t.is_floating_point()) for t in acts)
                recvd = recvd[0] if len(recvd) == 1 else recvd
    ...
    def _exec_recv_grads(self, buffer_id):
        # MoEvement recovery: use logged gradients instead of blocking P2P receive
        if self.moevement_coordinator is not None and self.moevement_coordinator.is_recovering():
            grads = self.moevement_coordinator.get_replay_gradients(self.global_steps, buffer_id)
            if grads is not None:
                # Ensure grad_layer buffers exist; allocate from logged shapes if needed
                if self.grad_layer is None or self.dynamic_shape:
                    if len(grads) == 1:
                        self.grad_layer = grads[0].to(self.device).clone()
                    else:
                        self.grad_layer = [g.to(self.device).clone() for g in grads]
                else:
                    if isinstance(self.grad_layer, torch.Tensor):
                        self.grad_layer.copy_(grads[0].to(self.device))
                    else:
                        for buf, g in zip(self.grad_layer, grads):
                            buf.copy_(g.to(self.device))
                return
    ...
    def _exec_optimizer_step(self, lr_kwargs=None):
        ...
        # boundary's snapshot D2H (so optim.step won't write the master
        # / optim state the side-stream copy is still reading) and, in
        # recovery, zero weight gradients on frozen operators (so the
        # optimizer step leaves them unchanged — mirror of
        # ``DeepSpeedEngine.step`` at engine.py:2878).  ``train_batch``
        # bypasses the parent ``step``, so this call is mandatory in
        # steady state too — gating it on ``is_recovering()`` left the
        # snapshot D2H without a main-stream consumer wait, voiding the
        # ordering guarantee the deferred-sync path relies on.  The
        # recovery-specific ``zero_frozen_gradients`` is gated inside
        # the coordinator method itself.
        if self.moevement_coordinator is not None:
